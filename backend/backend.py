...
import time
from FasterRCNN_model import config
from keras import backend as K
from keras.layers import Input
from keras.models import Model
from FasterRCNN_model import roi_helpers

sys.setrecursionlimit(40000)

# resize image based on max size of image provided in config file
...
def upload_file():
	...
	data_dict = request.form.to_dict()  # changing formData to dictionary data
	data = (data_dict['file'].split(',')[1]) # extracts image data from dictionary
	w, h = (data_dict['imgDimensions'].split(',')) # extracts image dimensions from formData
	w = int(w)
	h = int(h)
	imgdata = base64.b64decode(data)
	img2 = Image.open(io.BytesIO(imgdata))
	a = np.array(img2.getdata()).astype(np.float64) # converting image data type to numpy
	print('type of data to model ', type(a))
	data = a.reshape(h,w,3)  # reshape image to original dimension
	...
	if C.network == 'resnet50':
		import FasterRCNN_model.resnet as nn
	elif C.network == 'vgg':
		import FasterRCNN_model.vgg as nn
